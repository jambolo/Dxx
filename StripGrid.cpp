/** @file *//********************************************************************************************************
        ...
        $Header: //depot/Libraries/Dxx/StripGrid.cpp#10 $
...
namespace Dxx
{
//! This function generates an array of vertex indexes for stripping a @a w by @a h grid of vertexes. The strip is
//! created like this:
...
//!
//! @param	w		Width of the grid (note: this is the number of vertexes -- the number of quads is @a w - 1)
//! @param	h		Height of the grid (note: this is the number of vertexes -- the number of quads is @a h - 1)
//! @param	pData	Where to place the indexes. Index values range 0 - 65535. The buffer must be big enough
//!					to hold at least ( @a h - 1 ) * ( 2 * @a w + 2 ) - 2 indexes.
//!
//! @return			Number of vertex indexes generated: ( @a h - 1 ) * ( 2 * @a w + 2 ) - 2
//!
//! @note		@a w * @a h must be less than 65536
...
//! The vertexes are stripped for a CW front-face.
//!
//! @param	(w-1)		Width of the grid (note: this is the number of vertexes -- the number of quads is @a w - 1)
//! @param	(h-1)		Height of the grid (note: this is the number of vertexes -- the number of quads is @a h - 1)
//! @param	pData	Where to place the indexes. The buffer must be big enough to hold at least
//!					( @a h - 1 ) * ( 2 * @a w + 2 ) - 2 indexes.
//!
//! @return			Number of vertex indexes generated: ( @a h - 1 ) * ( 2 * @a w + 2 ) - 2
//!
